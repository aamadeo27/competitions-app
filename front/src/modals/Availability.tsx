...
import { userQuery, timeframeCreate, timeframeUpdate, timeframeDelete } from '../graphql'
import { useUser } from '../logic/client'
import Loading from '../components/Loading'
import { TimeFrame, User } from '../generated/graphql'
...
export default function AvailabilityModal({ close }: Props) {
  ...
  const updateTimeframe = async () => {
    await tfUpdate({
      variables: {
        id: tfFields?.id,
        data: pick(tfFields,'userId','description','days','start','end','canPlay')
      }
    })
    userResult.refetch()
  ...
  const addLabel = <div className='flex flex-row w-full text-sm'>
    ...
    <div className={MAIN_CLASSES}>
      <div className='flex-none w-[40vw]'>
        ...
        <span className='text-sm text-gray-500 block'>
          Your availability determines the times when you could play
          a game with an opponent. It consists of a list of time frames,
          you can visualize your availability in the Challenge view of the Calendar.
        </span>
      </div>
      ...
      <div className={TF_FORM_CLASSES}>
        ...
        {tfFields && (
          <><div>
            <Input value={tfFields.description ?? ''} onChange={onChangeString('description')} label='Description' size='big'/>
          </div>

          <div>
            <span className='py-4 text-gray-500 inline-block'>Days which this timeframe is for</span>
            <DaySelector value={tfFields.days} onChange={onChangeString('days')} />
          </div>
