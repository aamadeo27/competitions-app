...
export default function DaySelector({ value, onChange }: Props) {
  ...
  const classes = (markedDay: boolean, d: string) =>
    classNames(
  ...
  const elements = DAY_ORDER.map((d) => (
    <div className={classes(marked.has(d), d)} key={d} onClick={onToggle(d)}>
      <Tooltip content={DAY[d as keyof typeof DAY]} />
      {d.toUpperCase()}
    </div>
  ))

  return <div className="flex flex-row w-fit h-fit">{elements}</div>
}
