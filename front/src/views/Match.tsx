...
import { Civ, Player, getCiv, getSetting, Setting } from '../logic/data'


const styles = {
  setting: (isEmpty: boolean) => isEmpty
    ? 'p-1 font-bold text-indigo-100 text-center border-dashed border-2 border-blue-200 rounded-md'
    : '',
  civ: (isEmpty: boolean) => isEmpty
    ? 'p-1 font-bold text-indigo-100 text-center border-dashed border-2 border-blue-200 rounded-md'
    : '',
}
...
function MatchSetting({ setting, id, onDrop, clearSetting }: MatchSettingProps){
  return (
    <div className={styles.setting(!setting)}
      onDrop={onDrop}
...
export default function Match({ pickers }: MatchProps) {
  ...
  const onDropCiv = (player: Player) => (e: React.DragEvent<HTMLDivElement>) => {
  ...
  return (
    <div className={matchClass}>
      <MatchSetting setting={match.setting}
        onDrop={onDropSetting}
        id={settId}
        clearSetting={clearSetting} />
      <div className='grid grid-cols-2 gap-2'>
        <MatchCiv civ={match.a.civ}
          onDrop={onDropCiv('a')}
          player={'a'}
          clearCiv={clearCiv('a')}/>
        <MatchCiv civ={match.b.civ}
          onDrop={onDropCiv('b')}
          player={'b'}
          clearCiv={clearCiv('b')}/>
      </div>
