...
function CompetitorCell({ data }: { data: User }) {
  return (
    <>
      <img className="block relative h-16 w-16 mr-4" src={data.avatar} />
...
export default function Ranking({ players }: Props) {
  const variant = (i: number) =>
    i === 0 ? 'first' : i <= players.length - 3 ? 'default' : 'last'

  const positions = players.map((p, i) => (
    <RankingRow
      key={i}
      variant={variant(i)}
      values={[i + 1, <CompetitorCell data={p} key={i} />, p.games, p.score]}
    />
  ))

  return (
    <Container sizeClasses="mr-10">
