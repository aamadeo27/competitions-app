...
import type { CompetitionMatch, RoundResult } from '../../generated/graphql'
...
const midBoxesClasses = (box: keyof typeof midBoxesSizes, played: boolean) =>
  classNames(
    'flex flex-col white',
    midBoxesSizes[box],
    played ? 'py-3 gap-3' : ''
...
function PlayerBox({ played, data, ranking, player }: PlayerBoxProps) {
  ...
  return (
    <>
      {player === 'p1' && (
        <a className="block w-fit h-fit" href={`/profile/${pData.steamId}`}>
          <img className="m-2 p-1 h-fit w-24" src={pData.avatar} />
        </a>
      )}
      <div className={midBoxesClasses(player, played)}>
        ...
        {played ? (
          <>
            <span>{wins(data.results!, pData.steamId)}</span>
          </>
        ) : (
          <>
...
function ShortMatchComponent({ data, ranking }: MatchProps) {
  ...
  const boxClasses = (box: 'left' | 'right') =>
    classNames(
      'h-full',
      !start ? 'bg-gray-700' : played ? 'bg-green-2' : 'bg-blue-1 tex-lg',
  ...
  const points = [
    wins(data.results!, data.player1.steamId),
