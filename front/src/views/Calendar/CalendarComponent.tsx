...
import { RoundResult } from '../../generated/graphql'
...
function AvailabilityEvent({ data }: EventProps) {
  ...
  const imgs = []
  if (rival) imgs.push(<img src={rival.avatar} className='h-6 w-6 rounded-full' key='rival'/>)
  if (showHero) imgs.push(<img src={hero.avatar} className='h-6 w-6 rounded-full' key='hero'/>)
...
function MatchEvent({ data }: EventProps) {
  ...
  items.push(<img src={player1.avatar} className='h-4 w-4 rounded-full' key='p1'/>)
  items.push(<img src={player2.avatar} className='h-4 w-4 rounded-full' key='p2'/>)

  const p1Wins = results.reduce(
    (wins: number, r: RoundResult) => wins + (r.winner === competitor1 ? 1 : 0),
    0
  )
  const score = results.length && <div className='flex flex-col'>
    <span>{player1.name} {p1Wins}</span>
    <span>{player2.name} {results.length-p1Wins}</span>
  </div>
...
function CalendarEvent({ data }: EventProps){
  if (data.event.extendedProps.type === 'availability') return <AvailabilityEvent data={data} />
  if (data.event.extendedProps.type === 'match') return <MatchEvent data={data} />
